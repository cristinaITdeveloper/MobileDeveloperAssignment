...
class ConnectionManager: NSObject {
    ...
    func getItems(itemName: String?, courseType: String?, completionHandler: (( URLResponse?, NSError?, [Item]?) -> Void)?){
        guard let requestUrl = Globals.sharedGlobals.getItemsUrl(itemName: itemName, courseType: courseType) else {
            return
        }
        ...
        let task = Globals.sharedGlobals.sharedSession.dataTask(with: request,
                                                                           completionHandler: {data, response, error -> Void in
                                                                            ...
                                                                            if let parseJSON : Dictionary<String,AnyObject> = json as? Dictionary<String,AnyObject> {
                                                                                ...
                                                                                let errorToShow = NSError(domain: "Errore", code: 0, userInfo: [NSLocalizedDescriptionKey: "Errore Interno, contattare amministratore"])
